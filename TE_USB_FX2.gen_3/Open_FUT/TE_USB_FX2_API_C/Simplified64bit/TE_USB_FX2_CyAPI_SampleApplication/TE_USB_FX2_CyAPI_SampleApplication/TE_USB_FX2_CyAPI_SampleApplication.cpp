...
int GetFPGAstatus()
{
  ...
  if (TE_USB_FX2_SendCommand( Command, CmdLength, Reply, ReplyLength, 1000)) {
    cout << "Error" << endl;
    return -1;
  }
...
void SendFPGAcommand( enum MB_Commands Command2MB)
{
  byte Command[64], Reply[64];
  int CmdLength = 64;
  long ReplyLength = 64;
...
void ReadData_Throughput(unsigned int packets, int RX_PACKET_LEN)
{
  ...
  for (unsigned int i = 0; i < packets; i++)
  {
    ...
    if (TE_USB_FX2_GetData(data_temp, packetlen)) //+total_cnt
    {
...
void WriteData_Throughput(unsigned int packets, int TX_PACKET_LEN, unsigned int GetFpgaStatusFlag)
{
  ...
  for (unsigned int i = 0; i < packets; i++)
  {
    ...
    if (TE_USB_FX2_SetData(data_temp, packetlen)) //data+total_cnt
    {
      cout << "ERROR" << endl;
      break;
    }
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  while (m_sel != '0')
  {
    ...
    case '1' :
      ...
      break;
    case '2' :
	  cout << "Write the Buffer Size desired for EPx (power of 2 is better)" <<endl;
      ...
      cout << "computer. DataThroughput value expected is >20-30 Mbyte/s," <<endl;
      ...
      TimeOut = 1000;
      cin>>TimeOut; // Try to parse the string as an integer

      if (TE_USB_FX2_Open(0, TimeOut , BufferSize)==0)
    ...
    case '3' :
      ...
      if (TE_USB_FX2_Open(1, 1000, 131072)==0)
        cout << "Module is connected!"  <<endl; //&m_handle
      //if (TE_USB_FX2_Open(&m_handle, 0)!=0)
      else
        cout << "Module is not connected!" <<endl;
    ...
    case 'r' :
      ...
      ReadData_Throughput(PacketsNumberR,PacketLengthR);
    ...
    case 'w' :
      ...
      WriteData_Throughput(PacketsNumberW,PacketLengthW,GetStatusFPGAyn);
      break;
