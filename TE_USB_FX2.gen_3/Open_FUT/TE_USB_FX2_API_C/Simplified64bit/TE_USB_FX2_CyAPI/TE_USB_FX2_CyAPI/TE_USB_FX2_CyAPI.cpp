...
//(vID == 0x0bd0) && (pID == 0x0300)

static int VID = 0x0bd0;

static int PID = 0x0300;

static CCyUSBDevice *USBDeviceList = new CCyUSBDevice((HANDLE)0,CYUSBDRV_GUID,true);;



static CCyBulkEndPoint *BulkOutEP = NULL;

static CCyBulkEndPoint *BulkInEP = NULL;
...
TE_USB_FX2_CYAPI int TE_USB_FX2_Open (  int CardNumber, unsigned long TimeOut, int DriverBufferSize )
{
  ...
  //*USBDeviceList = new CCyUSBDevice((HANDLE)0,CYUSBDRV_GUID,true);
  ...
  if ( ((CardNumber>=0) && (CardNumber<CardCounted)) )
  {
    ...
    vID = USBDeviceList->VendorID;
    pID = USBDeviceList->ProductID;
    if ((((pID == PID) && (vID == VID)) == true))
	  /*Driver Buffer Creation*/
	  {
		  TE_USB_FX2_GetData_InstanceDriverBuffer ( USBDeviceList,  &BulkInEP , PI_EP6, TimeOut, DriverBufferSize);
		  TE_USB_FX2_SetData_InstanceDriverBuffer ( USBDeviceList,  &BulkOutEP, PI_EP8, TimeOut, DriverBufferSize);
		  return ST_OK;
	  }
...
TE_USB_FX2_CYAPI int TE_USB_FX2_SendCommand ( byte* Command, long CmdLength, byte* Reply, long ReplyLength, unsigned long Timeout)
{
  ...
  CCyBulkEndPoint *BulkOutEPc = NULL;
  CCyBulkEndPoint *BulkInEPc = NULL;
  ...
  for (int i=0; i<eptCount; i++)
  {
    ...
    {
	  //If the EndPoint EP1 OUT exists instantiate the abstract class in the concrete class CCyBulkEndPoint
      BulkInEPc = (CCyBulkEndPoint *) USBDeviceList->EndPoints[i];
	  //set the TimeOut to Timeout
      BulkInEPc->TimeOut=Timeout;
	  //use synchronous method XferData to send Command
      bResultCommand=BulkInEPc->XferData(Command, CmdLength);
  ...
  for (int i=1; i<eptCount; i++)
  {
    ...
    {
	  //If the EndPoint EP1 OUT exists instantiate the abstract class in the concrete class CCyBulkEndPoint
      BulkOutEPc = (CCyBulkEndPoint *) USBDeviceList->EndPoints[i];
	  //set the TimeOut to Timeout
      BulkOutEPc->TimeOut=Timeout;
	  //use synchronous method XferData to receive Reply
      bResultReply=BulkOutEPc->XferData(Reply, ReplyLength);
    }
  }
...
int TE_USB_FX2_GetData_InstanceDriverBuffer (CCyUSBDevice *USBDeviceList, CCyBulkEndPoint **BulkInEP, PI_PipeNumber PipeNo,unsigned long Timeout, int BufferSize)
{
...
int TE_USB_FX2_SetData_InstanceDriverBuffer (CCyUSBDevice *USBDeviceList, CCyBulkEndPoint **BulkOutEPx, PI_PipeNumber PipeNo,unsigned long Timeout, int BufferSize)
{
  ...
  for (int i=1; i<eptCount; i++)
  {
    ...
    if (bBulkR && bOutR)
    {
      *BulkOutEPx = (CCyBulkEndPoint *) USBDeviceList->EndPoints[i];
      (*BulkOutEPx)->TimeOut=Timeout;
      (*BulkOutEPx)->XferMode=XMODE_DIRECT;
      (*BulkOutEPx)->SetXferSize(DeviceDriverBufferSize);
